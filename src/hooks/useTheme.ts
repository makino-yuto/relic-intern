...
function applyTheme(color: ThemeColor, sat: number, lit: number) {
  ...
  const bgL = isDark ? adjL + 8 : Math.min(adjL + 48, 97);
  const bgS = isDark ? adjS * 0.10 : Math.min(adjS * 0.14, 16);

  root.style.setProperty('--background', `${color.h} ${Math.round(bgS)}% ${Math.round(bgL)}%`);
  root.style.setProperty('--muted',      `${color.h} ${Math.round(bgS * 1.3)}% ${Math.round(bgL - (isDark ? -4 : 3))}%`);
  root.style.setProperty('--border',     `${color.h} ${Math.round(bgS * 2.0)}% ${Math.round(bgL - (isDark ? -12 : 9))}%`);
  root.style.setProperty('--input',      `${color.h} ${Math.round(bgS * 2.0)}% ${Math.round(bgL - (isDark ? -12 : 9))}%`);
  root.style.setProperty('--card',       `${color.h} ${Math.round(bgS * 0.4)}% ${isDark ? Math.round(adjL + 10) : 99}%`);
